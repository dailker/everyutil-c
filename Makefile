CC ?= gcc
CFLAGS ?= -std=c99 -Wall -Wextra -Iinclude -fPIC
LDFLAGS ?=
AR = ar
RANLIB = ranlib
SRC_DIR = src
...
ifeq ($(OS),Windows_NT)
    ...
    CFLAGS += -DBUILDING_EVERYUTIL -DEVERYUTIL_EXPORTS
else
...
# New: Build each test as its own executable in build/testExecutables/{category}/
...
$(LIB_SHARED): $(OBJ_FILES)
	$(CC) -shared -o $@ $^ $(LDFLAGS)
ifeq ($(OS),Windows_NT)
	$(CC) -shared -o $@ $^ -Wl,--out-implib,$(LIB_IMPORT)
endif
...
$(TEST_EXEC_DIR)/%.exe: $(TEST_DIR)/%.c $(LIB_STATIC)
	@mkdir -p $(dir $@)
...
ifeq ($(OS),Windows_NT)
	...
	cp -r $(INCLUDE_DIR)/* $(PREFIX)/include
